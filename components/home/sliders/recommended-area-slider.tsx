...
import { RecommendedAreaCard } from "@/components/home/card/recommended-area-card";
import Link from "next/link";

interface RecommendedAreasSliderProps {
  areas: Area[];
  title?: string;
  description?: string;
  className?: string;
...
export function RecommendedAreasSlider({
  areas,
  title = "Top Areas",
  description = "Discover all recommended areas",
  className,
  showAllLink,
}: RecommendedAreasSliderProps) {
  ...
  if (displayedAreas.length === 0) return null;
  ...
  return (
    <section className={className}>
      <div className="mb-3 sm:mb-4 md:mb-6 px-4 sm:px-6 md:px-8 lg:px-12 flex justify-between items-start">
        <div>
          ...
          <p className="text-[10px] sm:text-xs md:text-sm text-muted-foreground mt-0.5 sm:mt-1">
            {displayedAreas.length} available
          </p>
        </div>
        {showAllLink && (
          <Link
      ...
      <div className="relative w-full">
        ...
        >
          <CarouselContent className="gap-2 sm:gap-3 md:gap-4 lg:gap-5">
            {displayedAreas.map((area) => (
              <CarouselItem
                key={area.documentId}
                className={cn(
                  "basis-1/2 xs:basis-1/2 sm:basis-1/3 lg:basis-1/4 xl:basis-1/5"
                )}
              >
                <Link href={`/areas/${area.slug}`}>
                  <RecommendedAreaCard area={area} />
                </Link>
              </CarouselItem>
            ))}
          </CarouselContent>
